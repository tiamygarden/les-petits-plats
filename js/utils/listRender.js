...
function createIngredients(recipes) {
    ...
    let html = ''
    map.forEach((value, ingredient) => html += `<li class="col-4"><button class="border-0 bg-primary text-white m-2 p-1 text-start w-100">${ingredient}</button></li>`)
...
function createAppliances(recipes) {
    ...
    let html = ''
    map.forEach((value, appliance) => html += `<li><a href="#">${appliance}</li>`)
    wrapper.innerHTML = html
}
...
function createUstensils(recipes) {
    ...
    let html = ''
    map.forEach((value, ustensil) => html += `<li><a href="#">${ustensil}</li>`)
    wrapper.innerHTML = html
}